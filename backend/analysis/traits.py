...
def _score_snp(snp: PanelSNP, genotype: str | None) -> SNPResult:
    ...
    # flipped one (e.g. DRD4 rs747302 CC↔GG). Compare CATEGORIES (not the full effect
    # dicts) so a homozygote whose two strands share a category is not falsely withheld.
    if is_strand_ambiguous(
        ...
        return SNPResult(
            ...
            trait_domain=snp.trait_domain,
            coverage_note=(
                f"{strand_caveat} {snp.coverage_note}" if snp.coverage_note else strand_caveat
            ),
            cross_module=snp.cross_module,
        )
